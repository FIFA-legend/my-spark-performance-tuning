...
object SerializationProblems {
  ...
  class LegalDrinkAgeChecker(legalAge: Int) extends Serializable {
    def processPeople() = people.map(_.age >= legalAge).collect().toList
  }
  val peopleChecker = new LegalDrinkAgeChecker(21)
  ...
  class PersonProcessor { // <- can't put Serializable here
    ...
    class DrinkingAgeFlagger(checker: Int => Boolean) {
      def flag() = {
        val local = checker // <- 2: capture the lambda
        people.map(p => local(p.age)).collect().toList
      }
    }
